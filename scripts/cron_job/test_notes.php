...
$query_get_exp = "SELECT `photo_event`.`event_id`, `photo_event`.`event_name`, `photo_event`.`event_num`, `cust_customers`.`cust_handle`, `cust_customers`.`cust_fname`, `cust_customers`.`cust_lname`, `cust_customers`.`cust_email`, `cust_customers`.`cust_thumb`, `photo_event_images`.`image_tiny` , `photo_event_images`.`image_folder`, `photo_event_notes`.`event_message`, `photo_event_notes`.`event_image`, 
...
ABS(TO_DAYS(`photo_event_notes`.`event_date`) - TO_DAYS(NOW())) AS `TestToday`
	...
	INNER JOIN `photo_event` 
		ON `photo_event`.`event_id` = `photo_event_notes`.`event_id` 
...
if($total_get_exp > 0){
	...
	//$mail -> IsSMTP();
		...
		foreach($bcc as $v)	$mail -> AddBCC($v);
		if($Image !== false) $mail -> AddEmbeddedImage($Image, $IName);
		$mail -> Body = $page;
		$mail->Send();
		$Emailsent = true;
		...
		$BioImage = false;
		$Image = false;
		if($row_get_exp['event_image'] != "") unlink($TempName);
		unset($Image);
		unset($IName);
		unset($BioImage);
		unset($Handle);
