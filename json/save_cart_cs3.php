...
$code = clean_variable($parsed->cart->{'-number'}, true);
$photo = clean_variable($parsed->cart->{'-photographer'}, true);
$email = clean_variable($parsed->cart->{'-email'}, true);
$cart = clean_variable($parsed->cart->{'-cart'}, true);
...
if($total_check == 0){
	$query_check = "INSERT INTO `photo_cust_favories` (`fav_code`,`fav_email`,`fav_occurance`,`fav_date`,`fav_others`,`fav_cart`) VALUES ('".$code.$photo."','$email','2',NOW(),'y','$cart');";
} else {
	$row_check = mysql_fetch_assoc($check);
	$query_check = "UPDATE `photo_cust_favories` SET `fav_cart` = '$cart' WHERE `fav_id` = '".$row_check['fav_id']."';";
}
$check = mysql_query($query_check, $cp_connection) or die(mysql_error());

ob_start();

$wrapper = array( "cart"=> array( "done"=> "true" ));
