...
<table border="<? echo $border; ?>" cellpadding="<? echo $padd; ?>" cellspacing="<? echo $spac; ?>" <? if($class){ echo 'class="'.$class.'"';}if($height){ echo ' height="'.$height.'"';} if($width){ echo ' width="'.$width.'"';} if($align){ echo ' align="'.$align.'"';}?>>
 ...
 <tr<? if($cell_col_1){ ?> bgcolor="#<? if(is_int($n/2)){ echo $cell_col_1; } else { echo $cell_col_2; } ?>" onmouseover="this.bgColor='#<? echo $cell_col_3; ?>'" onmouseout="this.bgColor='#<? if(is_int($n/2)){ echo $cell_col_1; } else { echo $cell_col_2;} ?>'" <? } ?>>
  <td width="20" valign="top"><? if($control_show){ ?>
   <input name="<? echo $var_names; ?>_items[]" type="checkbox" class="no_border" id="<? echo $var_names."_items_".$n; ?>" value="<? echo $records[$key][1]; ?>">
   <? } else { print("&nbsp;"); } ?></td>
  <? foreach(array_slice($records[$key],2) as $k => $v){ ?>
  <? if(strlen($v) > 30 && $k == 2){
	   		//$v = substr($v,0,30)."...";
	   } ?>
  <td valign="top" <? if($k != 0){echo 'onClick="clickthis('.$n.',\''.$var_names.'_items\');"';} ?>><? if(($k == 0 && $div_data)){ ?>
   ...
   <? }
	  if($button_text && $k == 0) { 
			if($button_url == false){?>
   <a href="#"<? if($button_target){ echo 'target="'.$button_target.'"';}?> onclick="javascript:set_form('form_','<? echo implode(",",$path).','.$records[$key][1]; ?>','view','<? echo $sort; ?>','<? echo (is_array($rcrd)) ? implode(",",$rcrd) : $rcrd; ?>');">
   <? } else { 
			echo '<a href="'.$button_url.'"';
			if($button_target)echo' target="'.$button_target.'"';
			echo '>';
			} echo $v; ?>
   </a>
   <? } else { echo $v; }
