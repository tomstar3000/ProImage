...
$getList->mysql("SELECT `photo_quest_book`.`email`, `photo_quest_book`.`promotion` FROM `photo_event` INNER JOIN `photo_quest_book` ON `photo_quest_book`.`event_id` = `photo_event`.`event_id` WHERE `photo_event`.`cust_id` = '$CustId' AND `photo_event`.`event_id` = '$EId'  ORDER BY `photo_quest_book`.`email` ASC;");
