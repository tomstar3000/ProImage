...
$getInfo->mysql("SELECT `page_text_text` FROM `web_pages` INNER JOIN `web_page_text` ON `web_page_text`.`page_id` = `web_pages`.`page_id` WHERE `web_pages`.`page_id` = '5';");
$getInfo = $getInfo->Rows(); ?>

<h1 id="HdrType2" class="CreateEvnt">
  <div>Product Manager Help</div>
</h1>
