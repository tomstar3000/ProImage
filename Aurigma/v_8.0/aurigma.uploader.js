...
var axVERSION = '8.0.33.0', jVERSION = '8.0.33.0';
var uploaderClassID = 'C6A47FBB-2ECA-430E-8466-5523772CA4FA', uploaderProgID = 'Aurigma.Uploader.8033.1';
var thumbnailClassID = 'F84A1F8A-89B2-411E-BA71-B6FFE295A64D', thumbnailProgID = 'Aurigma.Thumbnail.8033.1';
...
browser = AU.browser = new (function () {
    ...
    if (this.isFF) {
        var uMatch = a.match(/firefox\/(.*)$/), ffVersion;
        this.isAfterFF17 = (uMatch && uMatch.length > 1 && uMatch[1] > 16);                
    }

    this.isWindowsOS = (navigator.platform.indexOf("Win") > -1);
    if (a.indexOf("mac") > -1) {
        var version = /mac os x (\d+(?:[\.|_]\d+)+)/.exec(a);
        var versionNumber = version[1].replace(/[_\.]/g, '').slice(0, 3);

        this.isMacBefore107 = (versionNumber < 107);
    }
	...
	getJava7Installed: function () {
		...
		if (navigator.javaEnabled()) {
            ...
            if (ver != null && ver.length > 0) {
                for (var i = 0, maxi = ver.length; i < maxi; i++) {
                    if (AU.javaDetector.compareVersions(ver[i], reqVer)) {
                        result = true;
                        break;
                    }
                }
	...
	var getHtml = function () {
		...
		if (useObjectTag) {
			...
			if (!browser.isIE64) {
				...
				try {
					var jre = AU.javaDetector.getJREs();
					if (!jre || jre.length == 0) {
						attributes.codebase = window.location.protocol +
								"//java.sun.com/update/1.7.0/jinstall-7u4-windows-i586.cab#Version=" + javaVersion;
					}
				} catch (err) { }
			}
