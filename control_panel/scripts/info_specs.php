...
include $r_path.'scripts/save_specs.php'; 
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  ...
  <?php 
	...
	if($totalRows_get_specs != 0) { ?>
  <tr>
    <td><strong>Parent Spec: </strong></td>
    <td><?php if(!$is_enabled){	echo $row_get_specs['spec_name']; } else { ?>
      <select name="Parent Category" id="Parent_Category">
        <?php do { ?>
        <option value="<?php echo $row_get_specs['spec_id']; ?>"<?php if($row_get_specs['spec_id'] == $SPId){print(" selected=\"selected\"");}?>><?php echo $row_get_specs['spec_name']; ?></option>
  ...
  <?php
	...
	if($totalRows_get_prarent_id != 0) mysql_free_result($get_specs);
?>
  <tr>
    ...
    <td width="80%"><?php if(!$is_enabled){
      ...
      <?php } ?>
      <input type="hidden" name="Spec_Id" id="Spec_Id" value="<?php echo $SId;?>" />
      &nbsp;</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td><?php if($is_enabled){ ?>
	  ...
	  if($Image != ""){?>
      &nbsp;<a href="<?php echo $Spec_Folder; ?>/<?php echo $Image;?>" target="_blank">View</a>
      <?php } 
	  if($is_enabled){ ?>
      &nbsp;
