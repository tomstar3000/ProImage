...
?>
<script src="/Aurigma/v_5.1/iuembed.js" type="text/javascript"></script>
...
<div id="Sub_Header"> <img src="/control_panel/images/hdr_form_left.jpg" width="17" height="33" border="0" align="left" />
 ...
 <p id="Upload"><a href="#" onclick="setWarningAction();imageUploader1.Send();return false;">Upload</a></p>
 ...
 <p id="View">
  <select id="selectView" onchange="selectView_change();">
   <option value="0"<? if($Group === true) echo ' selected="selected"'; ?>>Thumbnails</option>
   <option value="1">Icons</option>
   <option value="2"<? if($Group === false) echo ' selected="selected"'; ?>>List</option>
...
var iu = new ImageUploaderWriter("ImageUploader1", 800,500);
...
<? } else { ?>
iu.addParam("RedirectUrl", "<? echo $_SERVER['PHP_SELF']; ?>?uploaded=true&id=<? echo time().$path[2]; ?><? if($token!="")echo "&".$token; ?>");
<? } } else { ?>
...
iu.addParam("Action", "<? echo $r_path; ?>scripts/save_file_uploader.php?folder=<? echo $Name; ?>&event=<? echo time().$path[2]; ?>&parnt_group_id=<? echo $PGroupId; ?>&group=<? echo urlencode($Group); ?>&id=<? echo $GId; if($token!="")echo "&".$token;?>");
