...
$items = array();
$items = $_POST['Supplies_items'];
function delete_vals($value){
	global $cp_connection;
	$query_get_remove = "SELECT `proj_supp_remove` FROM `proj_supplies` WHERE `proj_supp_id` = '$value'";
	...
	$upd = "UPDATE `proj_supplies` SET `proj_supp_remove` = '$new_val' WHERE `proj_supp_id` = '$value'";
