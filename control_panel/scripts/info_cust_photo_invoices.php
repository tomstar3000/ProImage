...
function form_submit(id){
	mywindow = window.open("/control_panel/image_updater.php?<? echo ($token!="") ? $token."&": ""; ?>image="+id+"&invoice=<? echo $InvEnc; ?>","","toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=0,resizable=0,width=300,height=250");
}
</script>
<div id="Sub_Header"> <img src="/control_panel/images/hdr_form_left.jpg" width="17" height="33" border="0" align="left" />
  ...
  <? if($Sent != "y"){ ?>
  <p id="Lab"><a href="#" onclick="javascript:set_form('form_','<?php echo implode(",",$path); ?>','ToLab','<?php echo $sort; ?>','<?php echo $rcrd; ?>');">To Lab</a>
    <? } ?>
  </p>
</div>
