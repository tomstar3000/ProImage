...
if(isset($_POST['Controller']) && $_POST['Controller'] == "Delete"){
	include $r_path.'scripts/del_prod_ratings.php';
}
?>
...
<div>
  <?php
	$query_get_reviews = "SELECT * FROM `prod_ratings` WHERE  `prod_id` = '$PId'";
	...
	do{
		...
		$records[$count][1] = $row_get_reviews['rating_id'];
		$records[$count][2] = $row_get_reviews['rating_name'];
		$records[$count][3] = $row_get_reviews['rating'];
	} while ($row_get_reviews = mysql_fetch_assoc($get_reviews)); 
	mysql_free_result($get_reviews);
	
	build_record_5_table('Ratings','Ratings',$headers,$sortheaders,$records,$div_data,$drop_downs,array(array('Delete Rating(s)','Delete','Delete',false)),false,'100%','0','0','0',false,false,$Rec_Style_1,$Rec_Style_2,$Rec_Style_3,$Rec_Style_4,$Rec_Style_5);
?>
</div>
