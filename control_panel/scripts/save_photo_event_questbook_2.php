...
$QId = $path[2];
$EID = (isset($_POST['Event_Id'])) ? clean_variable($_POST['Event_Id']) : 0;
...
if (isset($_POST['Controller']) && $_POST['Controller'] == "Save" && ($cont == 'add' || $cont == 'edit')) {
    ...
    foreach ($Emails as $k => $v) {
        $query_get_info = "SELECT * FROM `photo_quest_book` WHERE `email` = '$v'";
    ...
    /** 2013-11-18 */
//	if($cont == "add"){
//		$add = "INSERT INTO `photo_event_notes` (`event_id`,`event_note_name`,`event_days`,`event_before`,`event_date`,`event_image`,`event_image_id`,`event_message`,`event_recps`) VALUES ('$EID','$Name','$Days','$Before','$Date','$Contents','','$Desc','');";
//		$addinfo = mysql_query($add, $cp_connection) or die(mysql_error());
//		
//		$query_get_info = "SELECT `event_note_id` FROM `photo_event_notes` WHERE `event_id` = '$EID' ORDER BY `event_note_id` DESC LIMIT 0,1";
//		$get_info = mysql_query($query_get_info, $cp_connection) or die(mysql_error());
//		$row_get_info = mysql_fetch_assoc($get_info);
//		
//		array_push($path,$row_get_info['event_note_id']);
//	} else {
//		if($Image === true || ($Image === false && $_POST['Remove_Image'] == "true")){
//			$upd = "UPDATE `photo_event_notes` SET `event_id` = '$EID', `event_note_name` = '$Name',`event_days` = '$Days',`event_before` = '$Before',`event_date` = '$Date',`event_image` = '$Contents',`event_image_id` = '',`event_message` = '$Desc',`event_recps` = '' WHERE `event_note_id` = '$QId'";
//		} else {
//			$upd = "UPDATE `photo_event_notes` SET `event_id` = '$EID', `event_note_name` = '$Name',`event_days` = '$Days',`event_before` = '$Before',`event_date` = '$Date',`event_image_id` = '',`event_message` = '$Desc',`event_recps` = '' WHERE `event_note_id` = '$QId'";
//		}
