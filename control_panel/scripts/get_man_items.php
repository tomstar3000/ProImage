...
$query_get_items = "SELECT SUM(`orders_invoice_prod`.`invoice_prod_qty`) AS `invoice_prod_qty`, `prod_products`.`prod_number`, `prod_products`.`prod_name` FROM `orders_invoice` LEFT OUTER JOIN `orders_invoice_prod` ON `orders_invoice_prod`.`invoice_id` = `orders_invoice`.`invoice_id` LEFT OUTER JOIN `prod_products` ON `prod_products`.`prod_id` = `orders_invoice_prod`.`prod_id` LEFT OUTER JOIN `sellers` ON `sellers`.`sell_id` = `prod_products`.`sell_id` WHERE `sellers`.`sell_id` = '$man_id' AND `orders_invoice`.`invoice_date` >= '$first_date' AND `orders_invoice`.`invoice_date` < '$last_date'  GROUP BY `prod_products`.`prod_id` ASC ORDER BY `prod_products`.`prod_name` ASC";
