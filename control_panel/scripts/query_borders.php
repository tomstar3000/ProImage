...
if(isset($_POST['Controller']) && $_POST['Controller'] == "Delete"){
	include $r_path.'scripts/del_borders.php';
}
...
<div id="Form_Header">
  <div id="Add"><img src="/<?php echo $AevNet_Path; ?>/images/add.jpg" width="72" height="33" alt="add" onclick="javascript:set_form('','Prod,Bord','add','<?php echo $sort; ?>','<?php echo $rcrd; ?>');" /> </div>
...
<div id="Div_Records">
  <?php 
	...
	$query_get_products = "SELECT `prod_products`.*, `prod_availability`.`avail_name` FROM `prod_products` LEFT OUTER JOIN `prod_availability` ON `prod_availability`.`availability_id` = `prod_products`.`availability_id` WHERE `prod_service` = 'b' AND `prod_use` = 'y'".$add_where.$Sort_val;
  ...
  $pageNum_get_products = $rcrd[0];
	...
	do{
		...
		$parents[0] = $row_get_products['cat_id'];
			...
			$div_data[$n] = $row_get_products['prod_short'];
...
if ($pageNum_get_products > 0) { // Show if not first page ?>
    <a href="#" onclick="javascript:set_form('form_','Prod,Bord','query','<?php echo $sort; ?>','0,<?php echo $totalPages_get_products; ?>');" title="First Page" onmouseover="window.status='First Page';return true"  onmouseout="window.status='Done';return true">&laquo;</a> <a href="#" onclick="javascript:set_form('form_','Prod,Bord','query','<?php echo $sort; ?>','<?php echo max(0, $pageNum_get_products - 1).','.$totalPages_get_products; ?>');" title="Previous Page" onmouseover="window.status='Previous Page';return true"  onmouseout="window.status='Done';return true">&lt;</a>
    <?php } else if($totalRows_get_products>$maxRows_get_products) {// Show if not first page ?>
    <?php } ?>
    <?php if($totalRows_get_products>$maxRows_get_products){?>
    <?php /*echo ($startRow_get_products + 1); ?> - <?php echo min($startRow_get_products + $maxRows_get_products, $totalRows_get_products); ?> of <?php echo $totalRows_get_products ?>
	<?php } // Show if more than max records */
	if(($totalPages_get_products+1)<10){
		$addpages = $totalPages_get_products+1;
	} else {
		$addpages = 10;
	}
	if(($pageNum_get_products+1)<5){
		$start = 1;
	} else if(abs(($pageNum_get_products+1)-($totalPages_get_products+1))<=4) {
		$start = ($totalPages_get_products+1)-$addpages+1;
	} else {
		$start = ($pageNum_get_products+1)-4;
	}
	for($n=$start;$n<($start+$addpages);$n++){
		print ('<a href="#"  onclick="javascript:set_form(\'form_\',\'Prod,Bord\',\'query\',\''.$sort.'\',\''.($n-1).','.$totalPages_get_products.'\');" onmouseover="window.status=\'Page '.$n.'\'; return true;"  onmouseout="window.status=\'Done\'; return true;" title="Page '.$n.'" ');
		if($n!=($pageNum_get_products+1)){
			echo "";
		} else {
			echo "";
		}
    ...
    <a href="#" onclick="javascript:set_form('form_','Prod,Bord','query','<?php echo $sort; ?>','<?php echo min($totalPages_get_products, $pageNum_get_products + 1).','.$totalPages_get_products; ?>');" title="Next Page" onmouseover="window.status='Next Page';return true"  onmouseout="window.status='Done';return true">&gt;</a> <a href="#" onclick="javascript:set_form('form_','Prod,Bord','query','<?php echo $sort; ?>','<?php echo $totalPages_get_products.','.$totalPages_get_products; ?>');"  title="Last Page" onmouseover="window.status='Last Product';return true"  onmouseout="window.status='Done';return true">&raquo;</a>
    <?php } else if($totalRows_get_products>$maxRows_get_products){// Show if not last page ?>
    <?php }  ?>
  </div>
  <?php
	build_record_5_table('Borders','Borders',$headers,$sortheaders,$records,$div_data,$drop_downs,array(array('Delete Border(s)','Delete','Delete',false)),false,'100%','0','0','0',false,false,$Rec_Style_1,$Rec_Style_2,$Rec_Style_3,$Rec_Style_4,$Rec_Style_5);
$rcrd = implode(",",$rcrd);
?>
</div>
