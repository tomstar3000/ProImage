...
do{
	if(($row_get_reports['invoice_date'] <= $first_date || $row_get_reports['invoice_date'] >= $last_date) || $is_date == 0){
		...
		$records[$count][3] = $totalRows_get_customers;
...
build_record_table('Monthly_Reports', 'Monthly Reports', '', '', false, '', '', '', $headers, $sortheaders, $records, $div_data, $drop_downs, $_SERVER['PHP_SELF']."?Path=".$pathing."&Sort=".$Sorting, 'Open', '', '', 0, 0, 0, '', '100%', '', '',$Rec_Style_1, $Rec_Style_2, $Rec_Style_3, $btn_add, $hdr_img, "Record_Drop_Down");
?>
</div>