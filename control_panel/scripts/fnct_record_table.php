  ...
  <form method="post" action="<?php echo $_SERVER['PHP_SELF']."?".$_SERVER['QUERY_STRING']; ?>" name="<?php echo $var_names; ?>_form" id="<?php echo $var_names; ?>_form">
    ...
    <tr>
      <td colspan="<?php echo $cell_num+2; ?>"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="100%" align="left"><?php if($check_all_img){
			...
			echo "<img src=\"/".$AevNet_Path."/images/".$uncheck_all_img."\" name=\"Un-Check All\" onclick=\"changeselect(false,'".$var_names."_items','".$var_names."_count');\" alt=\"Un-Check All\" />";
              ...
              <?php } ?></td>
            <td align="right"><?php if($control_img){
				echo "<img src=\"/".$AevNet_Path."/images/".$control_img."\" name=\"".$control_val."\" onclick=\"confirmdelete(".$var_names."_form', '".$message."', 'true', '".$var_names."_controller');\" alt=\"".$control_val."\" />";
			} else { ?>
              <input type="button" name="<?php echo $control_val; ?>" id="<?php echo $control_val; ?>" value="<?php echo $control_val; ?>" onClick="confirmdelete('<?php echo $var_names; ?>_form', '<?php echo $message; ?>', 'true', '<?php echo $var_names; ?>_controller');">
              <?php } ?>
            </td>
          </tr>
        </table></td>
