...
if(isset($_POST['Controller']) && $_POST['Controller'] == "Delete"){
	include $r_path.'scripts/del_prod_specl.php';
}
?>
...
<div>
  <?php
	$query_get_special = "SELECT `prod_link_prod_spec_del`.*, `prod_special_delivery`.* FROM `prod_link_prod_spec_del` INNER JOIN `prod_special_delivery` ON `prod_special_delivery`.`spec_del_id` = `prod_link_prod_spec_del`.`spec_del_id` WHERE `prod_id` = '$PId' ORDER BY `prod_special_delivery`.`spec_del_name` ASC";
	...
	do{
		...
		$records[$count][1] = $row_get_special['link_prod_spec_del_id'];
		$records[$count][2] = $row_get_special['spec_del_name'];
		$records[$count][3] = "$".number_format($row_get_special['spec_del_price'],2,".",",");
		$records[$count][4] = ($row_get_special['required'] == "y") ? "Yes" : "No";
	} while ($row_get_special = mysql_fetch_assoc($get_special)); 
	
	mysql_free_result($get_special);
	
	build_record_5_table('Special Delivery','Special Delivery',$headers,$sortheaders,$records,$div_data,$drop_downs,array(array('Delete Delivery Option(s)','Delete','Delete',false)),false,'100%','0','0','0',false,false,$Rec_Style_1,$Rec_Style_2,$Rec_Style_3,$Rec_Style_4,$Rec_Style_5);
?>
</div>
