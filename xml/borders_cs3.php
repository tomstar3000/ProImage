...
$data1 = array("id" => $row_getImage['image_id'], "width" => 195, "height" => 195);
$data2 = array("id" => $row_getImage['image_id'], "width" => 630, "height" => 630);
$data3 = array("id" => $row_getImage['image_id'], "width" => 960, "height" => 640);
//$folder = substr($row_getImage['image_folder'],0,-11);
echo '		<image id="'.$row_getImage['image_id'].'" tiny="?data='.base64_encode(encrypt_data(serialize($data1))).'" small="?data='.base64_encode(encrypt_data(serialize($data2))).'" zoom="?data='.base64_encode(encrypt_data(serialize($data3))).'" folder="images/image.php">'.$name.'</image>';
...
$query_getImage = "SELECT `prod_categories`.*
	...
	ORDER BY `prod_categories`.`cat_order` ASC";
$getImage = mysql_query($query_getImage, $cp_connection) or die(mysql_error());
echo '<categories>';
...
while($row_getImage = mysql_fetch_assoc($getImage)){
	...
	if($row_getImage['prod_image'] == ""){
		...
		echo '<horizontal small="'.$row_getImage['prod_thumb'].'" zoom="'.$row_getImage['prod_image'].'" x1="'.$row_getImage['prod_x1'].'" y1="'.$row_getImage['prod_y1'].'" x2="'.$row_getImage['prod_x2'].'" y2="'.$row_getImage['prod_y2'].'"></horizontal>';
	}
	if($row_getImage['prod_image2'] == ""){
		echo '<vertical>None</vertical>';
	} else {
		echo '<vertical small="'.$row_getImage['prod_thumb2'].'" zoom="'.$row_getImage['prod_image2'].'" x1="'.$row_getImage['prod_x3'].'" y1="'.$row_getImage['prod_y3'].'" x2="'.$row_getImage['prod_x4'].'" y2="'.$row_getImage['prod_y4'].'"	></vertical>';
	}
		echo '<prices>';
		$query_get_info = "SELECT `rel_ids` FROM `prod_relationships` WHERE `prod_id` = '".$row_getImage['prod_id']."'";
		$get_info = mysql_query($query_get_info, $cp_connection) or die(mysql_error());
		$row_get_info = mysql_fetch_assoc($get_info);
