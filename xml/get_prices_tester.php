...
$data = '<event number="demery" photographer="rsphoto"></event>';
...
$cust_email = clean_variable($tree['EVENT']['ATTRIBUTES']['EMAIL'],true);
$event_number = "031609";
$client_number = "eagleeyephotos";
$cust_email = "chad.serpan@aevium.com";
$group_id = 0;
					...
					(
						(`photo_evnt_mrkt`.`event_mrk_days_early_purchase` != '0' 
							...
							AND (`photo_evnt_mrkt`.`event_mrk_days_cart_expire` = '0' 
								OR ABS(TO_DAYS(`photo_event`.`event_date`) + `photo_evnt_mrkt`.`event_mrk_days_after_start` - TO_DAYS(NOW())) <= `photo_evnt_mrkt`.`event_mrk_days_cart_expire`)
							AND `photo_evnt_mrkt`.`event_mrk_starts` = 'y'
