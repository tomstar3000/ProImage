...
//$data = '<event number="Goldrush" photographer="RyanSerpan" email="development@proimagesoftware.com"></event>';
$data = '<event number="bss09w" photographer="flyinghorse" email="development@proimagesoftware.com"></event>';
...
echo "SELECT `event_id` FROM `photo_event` INNER JOIN `cust_customers` ON `cust_customers`.`cust_id` = `photo_event`.`cust_id` WHERE `event_num` = '$event_number' AND `cust_handle` = '$photographer' AND `event_use` = 'y' ORDER BY `event_id`";
...
if($total_check==0) die('<?xml version="1.0" encoding="iso-8859-1"?><success><action>failed</action><info photorapher="'.$photographer.'" number="'.$event_number.'"></info></success>');
$event_number = array();
$event_number2 = array();
...
function buildGroup($prntId, $n){
	...
	while($row_get_info = mysql_fetch_assoc($get_info)){
		echo '<group id="'.rawurlencode($row_get_info['group_id']).'" name="'.rawurlencode($row_get_info['group_name']).'">';
		buildGroup($row_get_info['group_id'], $n+1);
		echo '</group>';
	}
}
function buildImages(){
	global $cp_connection, $event_number2, $cookieName, $Email, $code, $perpage, $favsArry;
	...
	while($row_get_info = mysql_fetch_assoc($get_info)){
		...
		if($LGroup == NULL || $LGroup != $row_get_info['group_id']) { $c = 1; $p = 1; $LGroup = $row_get_info['group_id']; }
		echo '<image id="'.$row_get_info['image_id'].'" groupid="'.$row_get_info['group_id'].'" tiny="'.$row_get_info['image_tiny'].'" small="'.$row_get_info['image_small'].'" zoom="'.$row_get_info['image_small'].'" folder="'.$folder.'" page="'.$p.'" fav="'.((in_array($row_get_info['image_id'],$favsArry))?'y':'n').'">'.rawurlencode($name).'</image>';
		$c++;
		if($c > $perpage){ $p++; $c = 1; }
	}
	/*
...
echo '<event name="'.$row_get_info['event_name'].'" price_id="'.$row_get_info['event_price_id'].'" desc="'.str_replace($replace,$with,$row_get_info['event_short']).'" copy="'.$row_get_info['event_copyright'].'" opacity="'.$row_get_info['event_opacity'].'" freq="'.$row_get_info['event_frequency'].'"><groups>';
buildGroup(0, 1);
