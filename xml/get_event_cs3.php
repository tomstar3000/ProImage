...
function buildImages(){
	...
	while($row_get_info = mysql_fetch_assoc($get_info)){
		...
		echo '<image id="'.$row_get_info['image_id'].'" groupid="'.$row_get_info['group_id'].'" tiny="?data='.base64_encode(encrypt_data(serialize($data1))).'" small="?data='.base64_encode(encrypt_data(serialize($data2))).'" zoom="?data='.base64_encode(encrypt_data(serialize($data3))).'" folder="images/image.php" page="'.$p.'" fav="'.((in_array($row_get_info['image_id'],$favsArry))?'y':'n').'">'.rawurlencode($name).'</image>';
		$c++;
		if($c > $perpage){ $p++; $c = 1; }
	}
	/*
