...
$disc = $tree['EVENT']['ATTRIBUTES']['DISC'];

$query_get_disc = "SELECT `prod_discount_codes`.*, SUM(`orders_invoice_codes`.`disc_total`) AS `disc_total` 
	...
	WHERE `cust_handle` = '$photographer'
		...
		AND `disc_use` = 'y'
	GROUP BY `orders_invoice_codes`.`disc_id`";
...
while($row_get_disc = mysql_fetch_assoc($get_disc)){
	echo '<discount id="'.$row_get_disc['disc_id'].'" name="'.$row_get_disc['disc_name'].'" code="'.$row_get_disc['disc_code'].'" price="'.$row_get_disc['disc_price'].'" perc="'.$row_get_disc['disc_percent'].'" buy="'.$row_get_disc['disc_item'].'" get="'.$row_get_disc['disc_for'].'" limit="'.$row_get_disc['disc_limit'].'" prodid="'.$row_get_disc['prod_id'].'" rollover="'.$row_get_disc['disc_roll_over'].'" rolltotal="'.$row_get_disc['disc_total'].'" image="'.(($row_get_disc['disc_image'] != "" ) ? '../images/discounts/'.$row_get_disc['disc_image']:'').'"></discount>';
}
echo '</photographer>';
...
$query_get_disc = "SELECT `prod_discount_codes`.*
	...
	WHERE `photo_event`.`event_num` = '".$code."'
